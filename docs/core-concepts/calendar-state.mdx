...
*   **Default Behavior**: In the core library, if no `calendarId` is provided, it defaults to `'default-calendar'`. However, most framework wrappers (React, Vue, Solid, etc.) will automatically generate a unique, stable ID for each component instance if one is not provided, ensuring isolated state out-of-the-box.
