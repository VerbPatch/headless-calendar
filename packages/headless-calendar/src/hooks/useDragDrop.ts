...
export const useDragDrop = (options: UseDragDropOptions = {}): UseDragDropReturn => {
   ...
   */
  const startDrag = createCallback(
    (event: CalendarEvent, dragData: Record<string, any> = {}): void => {
      setDraggedEvent({
        event,
        type: 'event',
        ...dragData,
   ...
   */
  const handleDrop = createCallback(
