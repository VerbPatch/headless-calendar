    ...
    describe('Validation and Management', () => {
        ...
        describe('Recurring Event Validation', () => {
            ...
            it('should validate daily recurrence', () => {
                ...
                expect(validateEvent(invalid)).toContain('Daily recurrence should not include weekDays, day, week, or month properties');
            });
            ...
            it('should validate weekly recurrence', () => {
                ...
                expect(validateEvent(invalid)).toContain('Weekly recurrence must specify weekDays array with at least one day');
            });
            ...
            it('should validate monthly recurrence complex rules', () => {
                ...
                expect(validateEvent(invalid)).toContain('Monthly recurrence with "week" must also specify "weekDays"');

                const onlyWeekDays: any = {
                    ...event1,
                    recurring: { repeat: 'monthly', every: 1, weekDays: [1] }
                };
                expect(validateEvent(onlyWeekDays)).toContain('Monthly recurrence with "weekDays" must also specify "week"');

                const both: any = {
                    ...event1,
                    recurring: { repeat: 'monthly', every: 1, day: 1, week: 1, weekDays: [1] }
                };
                expect(validateEvent(both)).toContain('Monthly recurrence cannot specify both "day" and "weekDays/week" properties');

                const none: any = {
                    ...event1,
                    recurring: { repeat: 'monthly', every: 1 }
                };
                expect(validateEvent(none)).toContain('Monthly recurrence must specify either "day" or "weekDays" with "week"');

                const invalidDay: any = {
                    ...event1,
                    recurring: { repeat: 'monthly', every: 1, day: 0 }
                };
                expect(validateEvent(invalidDay)).toContain('Monthly "day" must be an integer between -31 and 31, excluding 0');

                const invalidWeek: any = {
                    ...event1,
                    recurring: { repeat: 'monthly', every: 1, week: 5, weekDays: [1] }
                };
                expect(validateEvent(invalidWeek)).toContain('Monthly "week" must be an integer: 1-4 for specific week, or -1 for last week');

                const manyDays: any = {
                    ...event1,
                    recurring: { repeat: 'monthly', every: 1, week: 1, weekDays: [1, 2] }
                };
                expect(validateEvent(manyDays)).toContain('Monthly recurrence with "week" should typically specify only one day in "weekDays"');

                const emptyWeekDays: any = {
                    ...event1,
                    recurring: { repeat: 'monthly', every: 1, week: 1, weekDays: [] }
                };
                expect(validateEvent(emptyWeekDays)).toContain('Monthly "weekDays" must be a non-empty array');

                const invalidWeekDay: any = {
                    ...event1,
                    recurring: { repeat: 'monthly', every: 1, week: 1, weekDays: ['1'] }
                };
                expect(validateEvent(invalidWeekDay)).toContain('Monthly "weekDays" must contain integers between 0 (Sunday) and 6 (Saturday)');

                const withMonth: any = {
                    ...event1,
                    recurring: { repeat: 'monthly', every: 1, day: 1, month: 1 }
                };
                expect(validateEvent(withMonth)).toContain('Monthly recurrence should not include month property');
            });
            ...
            it('should validate yearly recurrence complex rules', () => {
                ...
                expect(validateEvent(invalidMonth)).toContain('Yearly recurrence must specify "month" as integer between 0 (Jan) and 11 (Dec)');

                const both: any = {
                    ...event1,
                    recurring: { repeat: 'yearly', every: 1, month: 0, day: 1, week: 1, weekDays: [1] }
                };
                expect(validateEvent(both)).toContain('Yearly recurrence cannot specify both "day" and "weekDays/week" properties');

                const none: any = {
                    ...event1,
                    recurring: { repeat: 'yearly', every: 1, month: 0 }
                };
                expect(validateEvent(none)).toContain('Yearly recurrence must specify either "day" or "weekDays" with "week"');

                const onlyWeek: any = {
                    ...event1,
                    recurring: { repeat: 'yearly', every: 1, month: 0, week: 1 }
                };
                expect(validateEvent(onlyWeek)).toContain('Yearly recurrence with "week" must also specify "weekDays"');

                const onlyWeekDays: any = {
                    ...event1,
                    recurring: { repeat: 'yearly', every: 1, month: 0, weekDays: [1] }
                };
                expect(validateEvent(onlyWeekDays)).toContain('Yearly recurrence with "weekDays" must also specify "week"');

                const invalidDay: any = {
                    ...event1,
                    recurring: { repeat: 'yearly', every: 1, month: 0, day: 40 }
                };
                expect(validateEvent(invalidDay)).toContain('Yearly "day" must be an integer between -31 and 31, excluding 0');

                const invalidWeek: any = {
                    ...event1,
                    recurring: { repeat: 'yearly', every: 1, month: 0, week: 5, weekDays: [1] }
                };
                expect(validateEvent(invalidWeek)).toContain('Yearly "week" must be an integer: 1-4 for specific week, or -1 for last week');

                const invalidWeekDay: any = {
                    ...event1,
                    recurring: { repeat: 'yearly', every: 1, month: 0, week: 1, weekDays: ['1'] }
                };
                expect(validateEvent(invalidWeekDay)).toContain('Yearly "weekDays" must contain integers between 0 (Sunday) and 6 (Saturday)');

                const emptyWeekDays: any = {
                    ...event1,
                    recurring: { repeat: 'yearly', every: 1, month: 0, week: 1, weekDays: [] }
                };
                expect(validateEvent(emptyWeekDays)).toContain('Yearly "weekDays" must be a non-empty array');

                const invalidYearlyDay: any = {
                    ...event1,
                    recurring: { repeat: 'yearly', every: 1, month: 0, day: 0 }
                };
                expect(validateEvent(invalidYearlyDay)).toContain('Yearly "day" must be an integer between -31 and 31, excluding 0');

                const manyDays: any = {
                    ...event1,
                    recurring: { repeat: 'yearly', every: 1, month: 0, week: 1, weekDays: [1, 2] }
                };
                expect(validateEvent(manyDays)).toContain('Yearly recurrence with "week" should typically specify only one day in "weekDays"');
            });
            ...
            it('should validate recurring metadata', () => {
                ...
                expect(validateEvent(invalidEvery)).toContain('Recurring "every" must be a positive integer');

                const invalidEnd: any = {
                    ...event1,
                    recurring: { repeat: 'daily', every: 1, end: 'invalid' }
                };
                expect(validateEvent(invalidEnd)).toContain('Recurring "end" must be a valid date if specified');

                const both: any = {
                    ...event1,
                    recurring: { repeat: 'daily', every: 1, count: 10, end: new Date() }
                };
                expect(validateEvent(both)).toContain('Cannot specify both "count" and "end" for recurring events');

                const earlyEnd: any = {
                    ...event1,
                    recurring: { repeat: 'daily', every: 1, end: new Date(2024, 0, 1) }
                };
                expect(validateEvent(earlyEnd)).toContain('Recurring "end" must be after the event end date');
            });
            ...
            it('should validate weekday lists', () => {
                ...
                expect(validateEvent(invalid)).toContain('Weekly "weekDays" must contain integers between 0 (Sunday) and 6 (Saturday)');
            });
        });
        ...
        it('should validate allDay event logic', () => {
            ...
            expect(validateEvent(invalid)).toContain('Event end date must be on or after start date for all-day events');
        });
