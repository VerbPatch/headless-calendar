...
$(document).ready(function () {
  $('#calendar').headlessCalendar({
    ...
    onRender: function (cal) {
      const { view, monthData, weekData, dayData, timeSlots, utils, events } = cal;
      ...
      events.forEach((e) => {
        ...
        $list.append(`
                      <li style="margin-bottom: 10px; border-bottom: 1px solid #eee; padding-bottom: 5px;">
                          ...
                          ${
                            e.recurrenceId
                              ? `<div style="font-size: 0.8em; color: #d63384margin-top:2px; margin-left: 15px;">Recurrence-ID:${typeof e.recurrenceId === 'string' ? e.recurrenceId : utils.formatDateTime(e.recurrenceId)}</div>`
                              : ''
                          }
                      </li>
                  `);
      ...
      if (view === 'month' && monthData) {
        ...
        monthData.weeks.forEach((week) => {
          ...
          week.forEach((date) => {
            ...
            const eventsForDate = cal.getEventsForDate(date);
            html += `
              <td style="
  ...
  $('#input-import').on('change', function (e) {
    ...
    if (file) {
      ...
      reader.onload = function (event) {
        ...
        $('#calendar').headlessCalendar('importFromICS', content);
      };
